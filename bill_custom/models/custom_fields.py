...
class SaleOrder(models.Model):
    ...
    def _compute_amounts(self):
        ...
        for order in self:
            ...
            order.amount_total_after_down_payment = order.amount_total - downpayment_total
            print("downpayment_total====",order.amount_untaxed,order.amount_total,order.amount_total_after_down_payment)

            # Protect from negative 
            # # or excessive downpayment
            # if downpayment_total < 0.0:
            #     downpayment_total = 0.0
            # if downpayment_total > amount_total:
            #     # cap at order total
            #     downpayment_total = amount_total

            # Write computed values on the order
            order.amount_untaxed = amount_untaxed
            order.amount_tax = amount_tax
            
            # ssssssss
            # order.amount_untaxed = before_down_payment - downpayment_total

            # down_payment_total = sum(order_lines.mapped('price_subtotal')) or 0.0
            order.down_payment = downpayment_total
            # self.amount_total_after_down_payment = order.amount_untaxed - downpayment_total
