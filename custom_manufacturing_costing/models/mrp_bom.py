...
class MrpBom(models.Model):
    ...
    def _onchange_subcontractor_ids(self):
        route_mto = self.env.ref('stock.route_warehouse0_mto')
        route_buy = self.env.ref('purchase_stock.route_warehouse0_buy')
        route_resupply_subcontract = self.env.ref(
            'mrp_subcontracting.route_resupply_subcontractor_mto'
        )

        if self.product_tmpl_id and self.subcontractor_ids:
            routes = self.product_tmpl_id.route_ids

            if route_mto not in routes:
                routes |= route_mto
            if route_buy not in routes:
                routes |= route_buy

            self.product_tmpl_id.route_ids = [(6, 0, routes.ids)]
        ...
        if self.subcontractor_ids:
            for line in self.bom_line_ids:
                product = line.product_id
                if product:
                    if route_resupply_subcontract not in product.route_ids:
                        product.write({
                            'route_ids': [(4, route_resupply_subcontract.id)]
                        })
